...
#
# Script generated by Vivado on Wed Apr 09 10:44:36 CEST 2025
...
#
# Usage        : dp_conv.sh
#                dp_conv.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                dp_conv.sh [-reset_run]
#                dp_conv.sh [-reset_log]
#                dp_conv.sh [-help]
#
...
#                script (type 'dp_conv.sh -help' for more information).
#
...
# script info
echo -e "dp_conv.sh - Script generated by export_simulation (Vivado v2023.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./dp_conv.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.dp_conv xil_defaultlib.glbl -o dp_conv_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./dp_conv_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key dp_conv_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc dp_conv_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./dp_conv.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: dp_conv.sh [-help]\n\
Usage: dp_conv.sh [-step]\n\
Usage: dp_conv.sh [-lib_map_path]\n\
Usage: dp_conv.sh [-reset_run]\n\
Usage: dp_conv.sh [-reset_log]\n\
Usage: dp_conv.sh [-keep_index]\n\
Usage: dp_conv.sh [-noclean_files]\n\n\
