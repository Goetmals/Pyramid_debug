...
architecture rtl of pyramid_dog is
	...
	signal delay00_en_s : std_logic;
	signal delay00_data_in_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);
	signal delay00_data_out_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);

	signal dog00_en_s : std_logic;
	signal dog00_x_s : std_logic_vector(xy_dog00_x_o'range);
	signal dog00_y_s : std_logic_vector(xy_dog00_y_o'range);
	signal dog00_data_a_s : std_logic_vector(xy_dog00_data_o'range);
	signal dog00_data_b_s : std_logic_vector(xy_dog00_data_o'range);

	signal delay01_en_s : std_logic;
	signal delay01_data_in_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);
	signal delay01_data_out_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);

	signal dog01_en_s : std_logic;
	signal dog01_x_s : std_logic_vector(xy_dog01_x_o'range);
	signal dog01_y_s : std_logic_vector(xy_dog01_y_o'range);
	signal dog01_data_a_s : std_logic_vector(xy_dog01_data_o'range);
	signal dog01_data_b_s : std_logic_vector(xy_dog01_data_o'range);

	signal delay10_en_s : std_logic;
	signal delay10_data_in_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);
	signal delay10_data_out_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);

	signal dog10_en_s : std_logic;
	signal dog10_x_s : std_logic_vector(xy_dog10_x_o'range);
	signal dog10_y_s : std_logic_vector(xy_dog10_y_o'range);
	signal dog10_data_a_s : std_logic_vector(xy_dog10_data_o'range);
	signal dog10_data_b_s : std_logic_vector(xy_dog10_data_o'range);

	signal delay11_en_s : std_logic;
	signal delay11_data_in_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);
	signal delay11_data_out_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);

	signal dog11_en_s : std_logic;
	signal dog11_x_s : std_logic_vector(xy_dog11_x_o'range);
	signal dog11_y_s : std_logic_vector(xy_dog11_y_o'range);
	signal dog11_data_a_s : std_logic_vector(xy_dog11_data_o'range);
	signal dog11_data_b_s : std_logic_vector(xy_dog11_data_o'range);

	signal delay20_en_s : std_logic;
	signal delay20_data_in_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);
	signal delay20_data_out_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);

	signal dog20_en_s : std_logic;
	signal dog20_x_s : std_logic_vector(xy_dog20_x_o'range);
	signal dog20_y_s : std_logic_vector(xy_dog20_y_o'range);
	signal dog20_data_a_s : std_logic_vector(xy_dog20_data_o'range);
	signal dog20_data_b_s : std_logic_vector(xy_dog20_data_o'range);

	signal delay21_en_s : std_logic;
	signal delay21_data_in_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);
	signal delay21_data_out_s : std_logic_vector(unsigned_num_bits(MAX_COLOR)-1 downto 0);

	signal dog21_en_s : std_logic;
	signal dog21_x_s : std_logic_vector(xy_dog21_x_o'range);
	signal dog21_y_s : std_logic_vector(xy_dog21_y_o'range);
	signal dog21_data_a_s : std_logic_vector(xy_dog21_data_o'range);
	signal dog21_data_b_s : std_logic_vector(xy_dog21_data_o'range);

	signal xy_grad_ready_s : std_logic;
	...
	signal xy_gauss00_ready_s : std_logic;
	...
	signal xy_gauss01_ready_s : std_logic;
	...
	signal xy_gauss02_ready_s : std_logic;
	...
	signal xy_subsample0_ready_s : std_logic;
	...
	signal xy_gauss10_ready_s : std_logic;
	...
	signal xy_gauss11_ready_s : std_logic;
	...
	signal xy_subsample1_ready_s : std_logic;
	...
	signal xy_gauss20_ready_s : std_logic;
	...
	signal xy_gauss21_ready_s : std_logic;
begin
	xy_grad_ready_s <= '1';
	xy_gauss00_ready_s <= '1';
	xy_gauss01_ready_s <= '1';
	xy_gauss02_ready_s <= '1';
	xy_subsample0_ready_s <= '1';
	xy_gauss10_ready_s <= '1';
	xy_gauss11_ready_s <= '1';
	xy_subsample1_ready_s <= '1';
	xy_gauss20_ready_s <= '1';
	xy_gauss21_ready_s <= '1';
	...
	pyramid_inst : component pyramid
		...
		port map (
			...
			xy_grad_ready_i => xy_grad_ready_s,
			...
			xy_gauss00_ready_i => xy_gauss00_ready_s,
			...
			xy_gauss01_ready_i => xy_gauss01_ready_s,
			...
			xy_gauss02_ready_i => xy_gauss02_ready_s,
			...
			xy_subsample0_ready_i => xy_subsample0_ready_s,
			...
			xy_gauss10_ready_i => xy_gauss10_ready_s,
			...
			xy_gauss11_ready_i => xy_gauss11_ready_s,
			...
			xy_subsample1_ready_i => xy_subsample1_ready_s,
			...
			xy_gauss20_ready_i => xy_gauss20_ready_s,
			...
			xy_gauss21_ready_i => xy_gauss21_ready_s
		);

	delay00_en_s <= xy_gauss00_en_s;
	delay00_data_in_s <= xy_gauss00_data_s;

	dog00_en_s <= xy_gauss01_en_s;
	dog00_x_s <= xy_gauss01_x_s;
	dog00_y_s <= xy_gauss01_y_s;
	dog00_data_a_s <= xy_gauss01_data_s;
	dog00_data_b_s <= delay00_data_out_s;
	...
	dog00 : component dog
		...
		port map (
			...
			xout => xy_dog00_x_o,
			yout => xy_dog00_y_o,
			data_in_a => dog00_data_a_s,
			data_in_b => dog00_data_b_s,
			data_out => xy_dog00_data_o
		);

	delay01_en_s <= xy_gauss01_en_s;
	delay01_data_in_s <= xy_gauss01_data_s;

	dog01_en_s <= xy_gauss02_en_s;
	dog01_x_s <= xy_gauss02_x_s;
	dog01_y_s <= xy_gauss02_y_s;
	dog01_data_a_s <= xy_gauss02_data_s;
	dog01_data_b_s <= delay01_data_out_s;
	...
	dog01 : component dog
		...
		port map (
			...
			xout => xy_dog01_x_o,
			yout => xy_dog01_y_o,
			data_in_a => dog01_data_a_s,
			data_in_b => dog01_data_b_s,
			data_out => xy_dog01_data_o
		);

	delay10_en_s <= xy_subsample0_en_s;
	delay10_data_in_s <= xy_subsample0_data_s;

	dog10_en_s <= xy_gauss10_en_s;
	dog10_x_s <= xy_gauss10_x_s;
	dog10_y_s <= xy_gauss10_y_s;
	dog10_data_a_s <= xy_gauss10_data_s;
	dog10_data_b_s <= delay10_data_out_s;
	...
	dog10 : component dog
		...
		port map (
			...
			xout => xy_dog10_x_o,
			yout => xy_dog10_y_o,
			data_in_a => dog10_data_a_s,
			data_in_b => dog10_data_b_s,
			data_out => xy_dog10_data_o
		);

	delay11_en_s <= xy_gauss10_en_s;
	delay11_data_in_s <= xy_gauss10_data_s;

	dog11_en_s <= xy_gauss11_en_s;
	dog11_x_s <= xy_gauss11_x_s;
	dog11_y_s <= xy_gauss11_y_s;
	dog11_data_a_s <= xy_gauss11_data_s;
	dog11_data_b_s <= delay11_data_out_s;
	...
	dog11 : component dog
		...
		port map (
			...
			xout => xy_dog11_x_o,
			yout => xy_dog11_y_o,
			data_in_a => dog11_data_a_s,
			data_in_b => dog11_data_b_s,
			data_out => xy_dog11_data_o
		);

	delay20_en_s <= xy_subsample1_en_s;
	delay20_data_in_s <= xy_subsample1_data_s;

	dog20_en_s <= xy_gauss20_en_s;
	dog20_x_s <= xy_gauss20_x_s;
	dog20_y_s <= xy_gauss20_y_s;
	dog20_data_a_s <= xy_gauss20_data_s;
	dog20_data_b_s <= delay20_data_out_s;
	...
	dog20 : component dog
		...
		port map (
			...
			xout => xy_dog20_x_o,
			yout => xy_dog20_y_o,
			data_in_a => dog20_data_a_s,
			data_in_b => dog20_data_b_s,
			data_out => xy_dog20_data_o
		);

	delay21_en_s <= xy_gauss20_en_s;
	delay21_data_in_s <= xy_gauss20_data_s;

	dog21_en_s <= xy_gauss21_en_s;
	dog21_x_s <= xy_gauss21_x_s;
	dog21_y_s <= xy_gauss21_y_s;
	dog21_data_a_s <= xy_gauss21_data_s;
	dog21_data_b_s <= delay21_data_out_s;
	...
	dog21 : component dog
		...
		port map (
			...
			xout => xy_dog21_x_o,
			yout => xy_dog21_y_o,
			data_in_a => dog21_data_a_s,
			data_in_b => dog21_data_b_s,
