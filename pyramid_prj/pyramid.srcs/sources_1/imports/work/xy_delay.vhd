...
begin
	...
	delay_x_follow_s <= std_logic_vector(to_unsigned(IMG_WIDTH-1, delay_x_follow_s'length)) when (DELAY_X = 0) else			
						std_logic_vector(to_unsigned(DELAY_X-1,xy_x_i'length));
						
	
	delay_y_follow_s <= std_logic_vector(to_unsigned(DELAY_Y-1,xy_y_i'length)) when (DELAY_X = 0) else
						std_logic_vector(to_unsigned(DELAY_Y,xy_y_i'length));
	...
	xy_en_s <= xy_en_i 	when (state_current_r = ST_FOLLOW) 	else
			   '1'		when (state_current_r = ST_END)		else
			   '0';
